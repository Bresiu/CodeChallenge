...
class UserCombinedModelToEntitiesMapper : Mapper.ObjectMapper<UserCombined, List<User>> {
    override fun map(`object`: List<User>): UserCombined {
        val userEntities = ArrayList<com.bresiu.codechallenge.data.entity.User>(`object`.size)
        val addressEntities = ArrayList<com.bresiu.codechallenge.data.entity.Address>(`object`.size)
        val geoEntities = ArrayList<com.bresiu.codechallenge.data.entity.Geo>(`object`.size)
        val companyEntities = ArrayList<com.bresiu.codechallenge.data.entity.Company>(`object`.size)
        for (user in `object`) {
            userEntities.add(
